...
The two previous problems would need to be solved very well first, and just having those two problems solved would aid developer productivity significantly already. I think it is possible to do this based on the unprecedented recent advances in AI predictive power and multi-modality, but it would take a significant amount of engineering effort and collaboration (not just one or two developers), and to get it to be capable of doing it reliably most of the time for simple features where the focus is on function rather than form, and with multi-modal LLMs and complex visual feedback and debugging systems it could even be used to help solve the further problem of properly turning figmas into usable (maintainable, acceptable-quality) code.
